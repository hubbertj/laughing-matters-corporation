...
var _createSessionTable = function() {
    ...
    client.connect(function(err) {
        ...
        client.query('CREATE TABLE IF NOT EXISTS "session" ("sid" varchar NOT NULL COLLATE "default", "sess" json NOT NULL, "expire" timestamp(6) NOT NULL) WITH (OIDS=FALSE);', [], function(error, result) {
            if (error) logger.error(error);
            logger.info('Create session table ');

            client.query('ALTER TABLE "session" ADD CONSTRAINT "session_pkey" PRIMARY KEY ("sid") NOT DEFERRABLE INITIALLY IMMEDIATE;', [], function(err, results) {
                if (err) {
